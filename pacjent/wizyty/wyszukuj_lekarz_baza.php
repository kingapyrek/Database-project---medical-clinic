<?php
    ...
    if($_SESSION['role'] == 'pacjent')
    {
        ...
        if(($_POST['spec'])!="0" && empty($_POST['nazwisko']))
        {
            //echo $_POST['spec'];
            $option="tylko spec";
          //  echo $option;
        }
        else if(!empty($_POST['spec'])  && !empty($_POST['nazwisko']) )
        {
            $option ="oba";
         //   echo $option;
        }
        else if(($_POST['spec'])==0 && !empty($_POST['nazwisko']))
        {
            $option = "tylko nazwisko";
           // echo $option;
        }
...
<br>
                        <form  method="post">
                            <div class="col-md-4 form-group">
                                <select class="form-select p-2" name="spec" aria-label="Default select example">
                        ...
                        <?php
                            ...
                            if($option == "tylko spec")
                            {                            
                                $query = pg_query_params($dbconn, 'SELECT * from wyszukaj_lekarz WHERE nazwa=$1 ORDER BY dzien, poczatek', array($_POST['spec'])) or die('Query failed: ' . pg_last_error());;
                            }
                            else if ($option =="oba")
                            {
                                $query = pg_query_params($dbconn, 'SELECT * from wyszukaj_lekarz WHERE nazwa=$1 AND nazwisko ILIKE $2 ORDER BY dzien, poczatek', array($_POST['spec'], '%' . $_POST['nazwisko'] . '%')) or die('Query failed: ' . pg_last_error());;

                            }
                            else if ($option == "tylko nazwisko")
                            {
                                $query = pg_query_params($dbconn, 'SELECT * from wyszukaj_lekarz WHERE nazwisko ILIKE $1 ORDER BY dzien, poczatek', array('%' . $_POST['nazwisko'] . '%')) or die('Query failed: ' . pg_last_error());;
                            }
